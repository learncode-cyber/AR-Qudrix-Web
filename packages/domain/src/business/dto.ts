import { z } from "zod";
import { BusinessCategory, BusinessStatus } from "@arqudrix/db/client-enums";

const SUPPORTED_LOCALES = ["en", "ar"] as const;

/**
 * A blank text input submits as "" (empty string), not `undefined` — plain
 * `.optional()` only skips validation for `undefined`, so `.url().optional()`
 * would otherwise reject an intentionally-blank image/website URL field.
 * Treat "" the same as "not provided" before the URL check ever runs.
 */
const optionalUrl = z.preprocess(
  (value) => (value === "" ? null : value),
  z.string().url("Must be a valid URL, e.g. https://example.com/image.jpg").nullable().optional()
);
